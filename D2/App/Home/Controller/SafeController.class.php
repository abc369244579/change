    ...
    public function ModifyPass(){
        ...
        if(IS_POST){
            ...
            if(IS_AJAX){
                ...
                $user = M('User');
                $password = I('password'); //用户输入的旧密码
                $old = $user->where(['id'=>$id])->find()['password'];
        ...
        if(IS_POST){
            ...
            if(!password_verify($password,$hash)){
                ...
                if($user->where(['id'=>$id])->save($data)){
                    $res = sendMail($data['email'],'傻逼','重新设定邮箱。<br/>请在30min内点击该链接。<br/><a href="http://120.78.178.244/D2/Home/User/newEmail/mark/'.$data['mark'].'">点击激活</a>');
                    if($res){
