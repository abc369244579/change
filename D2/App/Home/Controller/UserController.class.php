    ...
    public function back(){
        if(IS_POST){
            ...
            if($user->where(['id'=>$arr['id']])->save($mark)){
                $res = sendMail($arr['email'],'傻逼','重新设定密码。<br/>请在30min内点击链接重新您账号的密码。<br/><a href="http://120.78.178.244/D2/Home/User/backPass/mark/'.$mark['mark'].'">http://120.78.178.244/D2/Home/User/backPass/mark/'.$mark['mark'].'</a>');
                if($res){
